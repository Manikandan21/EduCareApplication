<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/AdminMaster.Master" AutoEventWireup="true" CodeBehind="Exam.aspx.cs" Inherits="ILogisticsApplication.Configuration.Exam" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPanel" runat="server">
            ...
            <div class="col-lg-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>Exam <small>Manage exam details</small></h5>
                        ...
                        <div class="row">
                            <div class="col-sm-12" id="divEditable" style="margin-bottom: 5px">
                                ...
                                <div class="form-group col-sm-6">
                                    ...
                                    <input type="text" id="txtExam" runat="server" placeholder="Enter exam" maxlength="15" class="form-control" />
                                </div>
                                <div class="form-group  col-sm-6">
                                    <label>Exam Type <span class="text-danger">*</span></label>
                                    <asp:DropDownList ID="ddlExamType" runat="server" class="form-control">
                                        <asp:ListItem Value="0">--Select--</asp:ListItem>
                                        <asp:ListItem Value="1">Marks & Grade</asp:ListItem>
    ...
    <script type="text/javascript">
            ...
            try {
                ...
                if (Data != undefined) {
                    ...
                    $.each(Data, function (index, PARAM) {
                        JsonData.push([
                           index + 1, PARAM.Exam, PARAM.ExamTypeName, PARAM.Exam_Code,
        ...
        function Validate() {
            ...
            var Exam = $('#<%=txtExam.ClientID %>').val();
            var ExamType = $('#<%=ddlExamType.ClientID %>').val();

            if (Exam == '' || Number(ExamType) == 0) {
        ...
        function ClearText() {
            $('#<%=txtExam.ClientID %>').val('');
            $('#<%=ddlExamType.ClientID %>').val(0);
            $('#<%=hdnId.ClientID %>').val('');
        ...
        function OnEdit(id) {
            try {
                ...
                if (Data != undefined) {
                    Data = jQuery.parseJSON(Data);
                    $.each(Data, function (index, PARAM) {
                        $('#<%=txtExam.ClientID %>').val(PARAM.Exam);
                        $("#<%=ddlExamType.ClientID %>").val(PARAM.ExamType).change();

                        $('#<%=hdnId.ClientID %>').val(PARAM.Id);
                        $('#<%=hdnMode.ClientID %>').val('Update');
                    });
                    $("#headEditable").text("Currently you are editing an exam is " + $('#<%=txtExam.ClientID %>').val());
                }
            }
