...
namespace ILogisticsApplication.Academic
{
    public partial class Academic : System.Web.UI.Page
    {
        ...
        private void BindData(string course, string section, string Mode)
        {
            try
            {
                ...
                else
                {
                    ...
                    grdAcademic.DataSource = lstAcademicCommonEntity;
        ...
        protected void grdAcademic_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        private void SaveAcademic(string Mode)
        {
            try
            {
                ...
                ObjAcademicDetailsEntity.ExamId = Convert.ToInt32(ddlExam.SelectedItem.Value.ToString());
                ObjAcademicDetailsEntity.CreatedBy = Convert.ToInt32(Session["UserId"].ToString());
                ObjAcademicDetailsEntity.Mode = Mode;
                if (Mode != "Delete")
                {
                    foreach (GridViewRow gvr in grdAcademic.Rows)
                    {

                        ObjMarkSet = new MarkSet();

                        ObjMarkSet.SubjectId = Convert.ToInt32(((Label)gvr.Cells[1].FindControl("lblSubjectId")).Text);
                        ObjMarkSet.Marks = Convert.ToInt32(((TextBox)gvr.Cells[2].FindControl("txtMarks")).Text.Trim());


                        lstMarkSet.Add(ObjMarkSet);
                    }
                    ObjAcademicDetailsEntity.MarkSet = lstMarkSet;
                }
        ...
        private void BindAcademicDetails(string course, string section, int ExamID, long StudentID, string Mode)
        {
            try
            {
                ...
                {                  
                    ...
                    grdAcademic.DataBind();
                    lblTotal.Text ="Total Marks: "+Convert.ToString(ObjAcademicDetailsEntity.TotalMarks);
